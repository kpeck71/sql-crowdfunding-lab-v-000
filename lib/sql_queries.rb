...
"SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS amount_over FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.titles HAVING amount_over <= 0" 
end
